<template>
  <nav class="navbar-container">
    <div
      ...
      :class="{
        'start-menu-depressed': $store.getters.getActiveWindow == 'Menu',
        'start-menu': $store.getters.getActiveWindow != 'Menu'
      }"
    >
      <div
        class=""
        :class="{
          'border': $store.getters.getActiveWindow == 'Menu',
          'container-border': $store.getters.getActiveWindow != 'Menu'
        }"
      >
        <img class="start-icon" src="@/assets/icons/win95/win95.png" />
        <button style="padding-left: 3px; padding-top: 5px; font-size: 0.9rem; font-weight: bold">
          <label>Start</label>
        </button>
      </div>
    </div>
  ...
  methods: {
    toggleStartMenu() {
      ...
      if (activeWindowId === 'Menu') {
        // If the Start Menu is already active, minimize it
        this.$store.commit("setActiveWindow", null);
      } else {
        // Otherwise, open the Start Menu
    ...
    minimizeOrOpenWindow(windowId) {
      ...
      if (activeWindowId === windowId) {
        ...
        // Otherwise, open the clicked window
